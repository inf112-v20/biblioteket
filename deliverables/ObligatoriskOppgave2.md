...
- Although we wanted to start working on the visual part of the game and get something that is playable, we quickly realized that we need to create a good back-end part of the game first. This is what we ended up working on in this iteration.
...
Our current definition of an MVP is a bare minimum implementation of the game, one in which the robot can move and interact with a subset of the available types of tiles and use simple cards that push it around. There should be a very simple GUI for this with a simple main menu and a HUD ingame that shows the current HP/damage and available cards. Interacting with the GUI is outside the scope of our MVP. This sprint we were able to make a very basic main menu but not GUI/HUD has been implemented in the current state of the game (noted in the bugs section).
...
- Since last time we have (see the appendix for the requirement list):
...
- Currently known bugs:
  - Player/Robot not completely synchronised with board and as such the players robot moves around freely without a care.

# Task 3 - Code

You can either run the project in your IDE of choice, if you're using IntelliJ IDEA you can simply just run the Main class. If you are using `maven` from the
command line you can package the game and its dependencies by running `mvn -B package` and then `java -jar target/roborally-1.0-SNAPSHOT-jar-with-dependencies.jar`.

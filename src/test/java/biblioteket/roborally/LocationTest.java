...
class LocationTest {
    ...
    void willMoveOfBoardToEastWhenXisLessThanMinWidth() {
        //assertTrue(location.willMoveOfBoard(BoardDirections.EAST));
    }

    @Test
    void willMoveOfBoardToEastWhenXisMoreThanMinWidth() {
        //assertFalse(location.willMoveOfBoard(BoardDirections.EAST));
    }

    @Test
    void willMoveOfBoardToSouthWhenYisLessThanMinHeight() {
        //assertTrue(location.willMoveOfBoard(BoardDirections.SOUTH));
    }

    @Test
    void willMoveOfBoardToSouthWhenYisMoreThanMinHeight() {
        //assertFalse(location.willMoveOfBoard(BoardDirections.SOUTH));
    }

    @Test
    void willMoveOfBoardToWestWhenXisMoreThanBoardWidth() {
        //assertTrue(location.willMoveOfBoard(BoardDirections.WEST));
    }

    @Test
    void willMoveOfBoardToWestWhenXisLessThanBoardWidth() {
        //assertFalse(location.willMoveOfBoard(BoardDirections.WEST));
    }

    @Test
    void willMoveOfBoardToNorthWhenYisMoreThanBoardHeight() {
        //assertTrue(location.willMoveOfBoard(BoardDirections.NORTH));
    }

    @Test
    void willMoveOfBoardToNorthWhenYisLessThanBoardHeight() {
        //assertFalse(location.willMoveOfBoard(BoardDirections.NORTH));
    }
    ...
    void locationInDirectionNORTH() {
        ...
        assertEquals(locationInDirection, location.locationInDirection(BoardDirections.NORTH));
    }

    @Test
    void locationInDirectionSOUTH() {
        Location locationInDirection = new Location(location.getX(),location.getY() - 1);
        assertEquals(locationInDirection, location.locationInDirection(BoardDirections.SOUTH));
    }

    @Test
    void locationInDirectionEAST() {
        Location locationInDirection = new Location(location.getX() - 1,location.getY());
        assertEquals(locationInDirection, location.locationInDirection(BoardDirections.EAST));
    }

    @Test
    void locationInDirectionWEST() {
        Location locationInDirection = new Location(location.getX() + 1,location.getY());
        assertEquals(locationInDirection, location.locationInDirection(BoardDirections.WEST));
    }
